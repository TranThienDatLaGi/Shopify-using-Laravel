        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            btn.addEventListener("click", async function () {
                ...
                try {
                    ...
                    const result = await response.json();
                    const res = await fetch(`/rules/${result.rule_id}?shop=${encodeURIComponent(SHOP)}`, {
                    ...
                    const productQuantity = result.product_quantity || 0;
                    if (batchId) {
                        await waitForBatchToFinish(rule, batchId, productQuantity);
                    }
            ...
            async function waitForBatchToFinish(rule, batchId, productQuantity) {
                ...
                while (attempts < maxAttempts) {
                    ...
                    const now = new Date();
                    // if (startTime && endTime && now >= startTime && now <= endTime && currentProgress >= 67) {
